        ...
        <div class="modal-panel">
          ...
          <div class="objetos-grid">
            ...
            >
              ...
              <p class="objeto-efecto-desc" v-if="store.jeringaDesbloqueada">Congela al crupier 1 turno</p>
            </div>
            ...
            >
              ...
              <p class="objeto-efecto-desc" v-if="store.encendedorDesbloqueado">Quema carta perjudicial</p>
            </div>
            ...
            >
              ...
              <p class="objeto-efecto-desc" v-if="store.puroDesbloqueado">Revela carta oculta del crupier</p>
            </div>
          </div>
...
<script setup>
  ...
  function toggleObjeto(nombre) {
    ...
    if (objetoActivo.value === 'pistola') { pistola.objetivo = null; pistola.efecto = null }
  }
  ...
  function cerrar() { objetoActivo.value = null; emit('close') }
...
<style scoped>
  ...
  .modal-panel {
    ...
    box-shadow: 0 0 60px rgba(180, 30, 0, 0.45), inset 0 0 80px rgba(0, 0, 0, 0.8);
    padding: 24px 32px;
  ...
  .modal-title { color: #cc2200; font-size: 1.1rem; letter-spacing: 5px; text-shadow: 0 0 10px rgba(200, 34, 0, 0.9); text-transform: uppercase; }
  .modal-close { background: none; border: 1px solid rgba(180, 40, 0, 0.4); color: #882200; font-size: 0.85rem; padding: 4px 10px; cursor: pointer; font-family: inherit; transition: all 0.2s; }
  ...
  .modal-mensaje { font-size: 0.82rem; color: #ff8800; text-align: center; letter-spacing: 1px; text-shadow: 0 0 8px rgba(255, 136, 0, 0.8); margin: 6px 0; text-transform: uppercase; }
  .modal-bloqueado { font-size: 0.78rem; color: #882200; text-align: center; letter-spacing: 2px; margin: 6px 0; animation: flicker 1.5s infinite; }
  .modal-hint { font-size: 0.62rem; color: #551100; text-align: center; letter-spacing: 2px; margin-top: 8px; text-transform: uppercase; }

  .seccion-titulo { font-size: 0.5rem; color: #882200; letter-spacing: 3px; text-align: center; margin: 8px 0 4px; text-transform: uppercase; }

  .premium-header { display: flex; align-items: center; justify-content: center; gap: 12px; margin: 4px 0; }
  .premium-lock-hint { font-size: 0.48rem; color: #664400; letter-spacing: 1px; }
  .premium-unlock-hint { font-size: 0.48rem; color: #ff8800; letter-spacing: 2px; text-shadow: 0 0 6px rgba(255,136,0,0.6); }

  .objetos-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin: 10px 0; }

  .objeto-card {
    display: flex; flex-direction: column; align-items: center;
    border: 1px solid rgba(120, 30, 0, 0.4); padding: 14px 10px 10px;
    background: rgba(20, 5, 0, 0.6); position: relative;
    transition: all 0.2s; min-height: 200px; justify-content: flex-start;
    overflow: visible;
  }
  .objeto-card--disponible { cursor: pointer; border-color: rgba(180, 40, 0, 0.5); }
  .objeto-card--disponible:hover { background: rgba(50, 10, 0, 0.8); border-color: rgba(255, 60, 0, 0.7); box-shadow: 0 0 20px rgba(200, 40, 0, 0.35); transform: translateY(-3px); }
  .objeto-card--activo { border-color: rgba(255, 80, 0, 0.9); box-shadow: 0 0 24px rgba(255, 60, 0, 0.45); background: rgba(60, 12, 0, 0.85); }
  .objeto-card--agotado { cursor: not-allowed; opacity: 0.45; filter: grayscale(60%); }
  .objeto-card--bloqueado { cursor: not-allowed; opacity: 0.28; filter: grayscale(85%); }

  .objeto-img-wrap { width: 140px; height: 110px; display: flex; align-items: center; justify-content: center; overflow: hidden; }
  .objeto-img { max-width: 100%; max-height: 100%; object-fit: contain; filter: drop-shadow(0 0 8px rgba(180, 40, 0, 0.5)); transition: filter 0.2s; }
  .objeto-card--disponible:hover .objeto-img { filter: drop-shadow(0 0 14px rgba(255, 80, 0, 0.75)); }
  .objeto-img--bloqueado { filter: grayscale(100%) brightness(0.35); }

  .objeto-nombre { margin: 8px 0 3px; font-size: 0.72rem; color: #cc3300; letter-spacing: 3px; text-transform: uppercase; text-shadow: 0 0 6px rgba(200, 51, 0, 0.7); }
  .objeto-cargas { font-size: 0.64rem; color: #882200; letter-spacing: 1px; margin: 0; }
  .objeto-efecto-desc { font-size: 0.44rem; color: #662200; letter-spacing: 1px; text-transform: uppercase; text-align: center; margin-top: 4px; }
  .objeto-lock { color: #441100; font-size: 0.65rem; }

  /* Submenú */
  .submenu { position: relative; bottom: auto; left: auto; transform: none; background: #0d0604; border: 1px solid rgba(200, 50, 0, 0.7); box-shadow: 0 0 24px rgba(180, 30, 0, 0.5); padding: 14px 16px; width: 100%; margin-top: 10px; z-index: 10; display: flex; flex-direction: column; gap: 8px; }
  .submenu-titulo { font-size: 0.62rem; color: #882200; letter-spacing: 2px; margin: 0 0 2px; text-transform: uppercase; }
  .submenu-fila { display: flex; gap: 8px; }
  .submenu-btn { flex: 1; background: rgba(40, 8, 0, 0.8); border: 1px solid rgba(150, 35, 0, 0.5); color: #cc4400; font-family: 'Courier New', Courier, monospace; font-size: 0.68rem; letter-spacing: 1px; padding: 6px 8px; cursor: pointer; text-transform: uppercase; transition: all 0.15s; }
  .submenu-btn:hover, .submenu-btn--sel { background: rgba(100, 20, 0, 0.9); border-color: rgba(255, 70, 0, 0.8); color: #ff5500; box-shadow: 0 0 8px rgba(255, 60, 0, 0.3); }
  .submenu-btn--confirmar { background: rgba(80, 15, 0, 0.9); border-color: rgba(180, 40, 0, 0.7); color: #ff4400; }
  .submenu-btn--confirmar:hover { background: rgba(140, 25, 0, 0.95); box-shadow: 0 0 10px rgba(255, 60, 0, 0.4); }
  .submenu-confirmar { margin-top: 4px; width: 100%; background: rgba(100, 20, 0, 0.9); border: 1px solid rgba(200, 50, 0, 0.7); color: #ff5500; font-family: 'Courier New', Courier, monospace; font-size: 0.68rem; letter-spacing: 2px; padding: 7px; cursor: pointer; text-transform: uppercase; transition: all 0.2s; }
  .submenu-confirmar:disabled { opacity: 0.35; cursor: not-allowed; }
  .submenu-confirmar:not(:disabled):hover { background: rgba(160, 30, 0, 0.95); box-shadow: 0 0 12px rgba(255, 60, 0, 0.5); }

  .modal-enter-active, .modal-leave-active { transition: opacity 0.2s, transform 0.2s; }
  .modal-enter-from, .modal-leave-to { opacity: 0; transform: scale(0.96); }
  .submenu-enter-active, .submenu-leave-active { transition: opacity 0.15s, transform 0.15s; }
